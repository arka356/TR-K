...
class KiwoomConditon(QObject):
    ...
    def __init__(self):
        ...
        self.jango_remove_keys = [
            ...
            '일{}봉'.format(user_setting.MAX_SAVE_CANDLE_COUNT),
            '{}분{}봉'.format(user_setting.REQUEST_MINUTE_CANDLE_TYPE, user_setting.MAX_SAVE_CANDLE_COUNT)]
    ...
    def waitingTradeSystemStateEntered(self):
        ...
        for name, info in condition_name_screenNo_dict.items():
            ...
            if (name == self.current_condition_name):
                ...
                if ('이탈' in name):
                    ...
                    print(
                        "stop condition " + name + ", screen_no: " + info[0] + ", nIndex " + '{}'.format(int(info[1])))
        ...
        for count in range(len(start_info_list)):
            print("start condition " + start_name_list[count] + ", screen_no: " + start_info_list[count][
                0] + ", nIndex " + '{}'.format(int(start_info_list[count][1])))
    ...
    def standbyProcessBuyStateEntered(self):
        ...
        QTimer.singleShot(30, self.sigRequestEtcInfo)
        pass
    ...
    def makeOpt10081Info(self, rQName):
        ...
        jongmok_info_dict['일{}봉'.format(user_setting.MAX_SAVE_CANDLE_COUNT)] = total_current_price_list[
                                                                               0:user_setting.MAX_SAVE_CANDLE_COUNT]
    ...
    def onTimerSystemTimeout(self):
        ...
        for key, value in user_setting.CONDITION_INFO.items():
            start_time = datetime.time(hour=value['start_time']['hour'], minute=value['start_time']['minute'],
                                       second=value['start_time']['second'])
            end_time = datetime.time(hour=value['end_time']['hour'], minute=value['end_time']['minute'],
                                     second=value['end_time']['second'])
            ...
            if (current_time > start_time and current_time < end_time):
                ...
                break
### LMH 추가 ###
        for key, value in user_setting.CONDITION_INFO_SELL.items():
            start_time = datetime.time(hour=value['start_time']['hour'], minute=value['start_time']['minute'],
                                       second=value['start_time']['second'])
            end_time = datetime.time(hour=value['end_time']['hour'], minute=value['end_time']['minute'],
                                     second=value['end_time']['second'])
...
### LMH 추가 ###
        ...
        for jongmok_code, value in self.maesu_wait_list.items():
            ...
            if (current_time > (jumun_time + time_span).time()):
                printData = "매수주문 취소 요청 {}, 주문번호: {}, {}".format(self.getMasterCodeName(jongmok_code), jumun_number,
                                                                 jumun_time)
                result = self.sendOrder("buy_" + jongmok_code, kw_util.sendBuyOrderScreenNo,
        ...
        if (realType == "주식호가잔량"):
            ...
            if ('분봉' in upjong):
                ...
                if (current_chegyeol_time_str != '장마감'):
                    current_chegyeol_time = datetime.datetime.strptime(current_chegyeol_time_str,
                                                                       "%H%M%S").time().replace(second=0)
                else:
                ...
                if (current_chegyeol_time >= (last_chegyeol_time + time_span).time().replace(second=0)):
                    upjong['분봉'].insert(0, '{}:{}'.format(current_price_str,
                                                          '19990101{}'.format(current_chegyeol_time_str)))
...
### LMH추가 ###
    ...
    * 14:03:36.353244 _OnReceiveChejanData gubun: 1, itemCnt: 34, fidList: 9201;9001;917;916;302;10;930;931;932;933;945;946;950;951;27;28;307;8019;957;958;918;990;991;992;993;959;924;10010;25;11;12;306;305;970
        ...
        if (gubun == "1"):  # 잔고 정보
            ...
            # 매도
                ...
                if (boyou_suryang == 0):
                    ...
                    chegyeol_info = util.cur_date_time('%Y%m%d%H%M%S') + ":" + str(current_price) + ":" + str(
                        current_amount)
            ...
            if (jumun_sangtae == "체결"):
                ...
                printData = ''
                if (jongmok_code in self.jangoInfo):
                    prinData = "sell: {} ordernumber: {}, 접수시간 {}, 가격 {}, 수량 {}".format(jongmok_name, jumun_number,
                                                                                        jumun_chegyeol_time,
                                                                                        jumun_price, jumun_qty)
                    self.jangoInfo[jongmok_code]['매도주문번호'] = jumun_number
                else:
                    if (jongmok_code not in self.maesu_wait_list):
                        printData = "buy: {} ordernumber: {}, 접수시간 {}, 가격 {}, 수량 {}".format(jongmok_name, jumun_number,
                                                                                            jumun_chegyeol_time,
                                                                                            jumun_price, jumun_qty)
                        ...
                        if (self.maesu_wait_list[jongmok_code]['매수주문번호'] == jumun_number):
                            printData = "cancel buy: {} ordernumber: {}, 접수시간 {}, 가격 {}, 수량 {}".format(jongmok_name,
                                                                                                       jumun_number,
                                                                                                       jumun_chegyeol_time,
                                                                                                       jumun_price,
                                                                                                       jumun_qty)
                            ...
                            printData = "cancel request buy: {} ordernumber: {}, 접수시간 {}, 가격 {}, 수량 {}".format(
                                jongmok_name, jumun_number, jumun_chegyeol_time, jumun_price, jumun_qty)
    ...
    def commGetData(self, jongmok_code, realType, fieldName, index, innerFieldName):
        return self.ocx.dynamicCall("CommGetData(QString, QString, QString, int, QString)", jongmok_code, realType,
...
if __name__ == "__main__":
    ...
    def test_sell():
        # 정상 매도 - kd 건설 1주
        objKiwoom.sendOrder("sell", kw_util.sendSellOrderScreenNo, objKiwoom.account_list[0],
                            kw_util.dict_order["신규매도"],
                            "044180", 1, 0, kw_util.dict_order["시장가"], "")
        pass
